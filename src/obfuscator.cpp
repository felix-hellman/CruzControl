...
int main() {
  ...
  auto rand =
      std::bind(std::uniform_int_distribution<int>(0, 255), std::mt19937(seed));
  std::vector<unsigned char> keyVector;

  for (int i = 0; i < KEYLENGTH; i++) keyVector.push_back(rand());

  std::ifstream input("src/strings.h");
